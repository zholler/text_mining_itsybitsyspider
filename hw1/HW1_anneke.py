...
class Corpus():
    ...
    def weighted_dict_rank(self, sentiment_dict, freq_rep = True ):
        """
        description: Uses an imported weighted dictionary to return a sentiment score for each speech
        freq_rep = True means we use document_term_matrix, tf_idf otherwise
        """
        # !!!it appears as though the order of these matches up but I'm not sure it always will!!!!!! 
        # perhaps we need to change into a list of tuples first...
        wordset = sentiment_dict.keys()
        values = afinn_dict.values()
        ...
        # what do we need to return? My guess: year, president, score (sorted by score). ie a list of tuples
        doc_list = []        
        for doc, i in zip(self.docs, range(len(self.docs))):
